...
class AuthTestCase(unittest.TestCase):
    ...
    def test_password_mismatch(self):
        """Test for when a user input a password that is not the same with the confirm_password"""
        self.user_data['password'] = 'password12'
        self.user_data['confirm_password'] = 'password1234'
        res = self.client().post('/api/auth/signup', data=self.user_data)
        # Get the results returned in json format
        result = json.loads(res.data.decode())
        # assert that the request contains a error message and a 400 status code
